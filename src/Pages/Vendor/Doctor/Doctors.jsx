...
export default function Doctors() {
  ...
  const handelSelectCountry = async (event) => {
    ...
    loginByRef.current.value="Select Doctor type"
  ...
  const handelloginBy = async () => {
    ...
    console.log(clientLoginBy);
    if (clientLoginBy === 'All') {
      getTokenDoctors()
    } else if (clientLoginBy === 'Select Status') {
      return false
    } else {
      await axios.post(`${process.env.REACT_APP_API_URL}/admin/doctors`, { IDPage: countDoctors, DoctorType: clientLoginBy }, apiheader).then((res) => {
        if (res.status === 200 && res.request.readyState === 4) {
          setFetchDoctors(res.data.Response.Doctors);
          setPagesCountDoctors(res.data.Response.Pages);
        }
                ...
                {!loadingDoctors ? <>
                  <Form.Group controlId="formBasicEmail"  > 
                    <Form.Select size="sm" aria-label="Default select example" ref={loginByRef} onChange={handelloginBy}  >
                      <option selected disabled hidden value={'Select Doctor type'}> {translate[isLang]?.filter?.loginBy}</option>
