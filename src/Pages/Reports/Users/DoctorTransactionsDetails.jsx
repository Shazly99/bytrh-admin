...
export default function DoctorTransactionsDetails() {
    let { id } = useParams()
    ...
    const getDoctorTransactionsDetails = async () => {
        await GetData(`${process.env.REACT_APP_API_URL}/admin/reports/doctor/transactions/details/${id}`, apiheader).then((res) => {
                ...
                <Row>
                    <Col xl={8} lg={8} md={8} sm={12} >
                        <div className="app__addprodects">
                            <Component.SubNav sub__nav={[{ name: translate[isLang]?.nav.navDoctor1, path: '/reports/doctors' },
                            { name: translate[isLang]?.nav.navDoctor2, path: `/reports/doctors/doctorTransactionsDetails/${id}` }]} />
                        </div>

                        <div className="payment-container">
                            <div className="payment-header d-flex gap-2 ">
                                <FaMoneyBillAlt className="payment-icon" />
                                <span className="payment-type">{translate[isLang]?.payment.title} </span>
                            </div>
                            <div className="payment-details row">
                                <Col xl={6} lg={6} md={12} sm={12}>
                                    <div className="payment-row">
                                        <div className="payment-label">{translate[isLang]?.payment?.label1}</div>
                                        <div className="payment-value">{dataDoctorTransactionsDetails.ClientName}</div>
                                    </div>
                                </Col>
                                <Col xl={6} lg={6} md={12} sm={12}>
                                    <div className="payment-row">
                                        <div className="payment-label">{translate[isLang]?.payment?.label4}</div>
                                ...
                                <Col xl={6} lg={6} md={12} sm={12}>
                                    <div className="payment-row">
                                        <div className="payment-label">{translate[isLang]?.payment?.label5}</div>
                                        {
                                ...
                                <Col xl={6} lg={6} md={12} sm={12}>
                                    <div className="payment-row">
                                        <div className="payment-label">{translate[isLang]?.payment?.label6}</div>
                                        {
                                ...
                                <Col xl={6} lg={6} md={12} sm={12}>
                                    <div className="payment-row">
                                        <div className="payment-label">{translate[isLang]?.payment?.label2}</div>
